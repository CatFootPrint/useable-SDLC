...
buffer_sdlc_error=Inf*ones(605,1);
buffer_ksvd_error=Inf*ones(605,1);
buffer_sdlc_time=Inf*ones(605,1);
buffer_ksvd_time=Inf*ones(605,1);
buffer_lambda=Inf*ones(605,1);
buffer_SNR=Inf*ones(605,1);
position=0;
for lambda=0:0.1:1
    for SNR=-10:2:10
        for counter=1:5
position=position+1;
buffer_SNR(position)=SNR;
buffer_lambda(position)=lambda;
parameter.SNR=SNR;
...
buffer_sdlc_error(position)=error_ratio_sdlc;
buffer_ksvd_error(position)=error_ratio_ksvd;
buffer_sdlc_time(position)=time_sdlc;
buffer_ksvd_time(position)=time_ksvd;
